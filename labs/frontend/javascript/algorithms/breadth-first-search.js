...
function breadthFirstSearch(graph, source) {
   ...
   // Step 2: Use the source to take the first element and put it into the queue
   queue.enqueue(source)
   // Initialize a set that tracks visited nodes.
   const visited = new Set()

   // Step 3: Iterate until the queue has no more items
   while (!queue.isEmpty()) {
      ...
      // Enqueue neighbours of the current node
